...
namespace Cheesebaron.MvxPlugins.SimpleWebToken.WindowsCommon
{
        ...
        : ISimpleWebToken
    {
        ...
        public string GenerateSignature(string unsignedToken, byte[] signingKey)
        {
            ...
            var bufferedUnsignedToken = CryptographicBuffer.ConvertStringToBinary(unsignedToken,
                BinaryStringEncoding.Utf8);

            var bufferedKey = CryptographicBuffer.CreateFromByteArray(signingKey);
            var hmacKey = provider.CreateKey(bufferedKey);
            var bufferedHmac = CryptographicEngine.Sign(hmacKey, bufferedUnsignedToken);
